...
class EAGLEWorker(TpModelWorker):
    ...
    def init_attention_backend(self):
        ...
        if self.speculative_algorithm.is_seagle() and self.server_args.attention_backend != "flashinfer":
            raise ValueError(
                f"SEAGLE is not supported in attention backend {self.server_args.attention_backend}, only flashinfer is supported"
            )
        ...
        if self.server_args.attention_backend == "flashinfer":
            ...
            self.draft_attn_backend = FlashAttentionMultiStepBackend(
                ...
                self.topk,
                self.speculative_num_steps,
                attention_sink=self.speculative_algorithm.is_seagle(),
            )
