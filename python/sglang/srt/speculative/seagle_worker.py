...
class EAGLEWorker(TpModelWorker):
    ...
    def init_cuda_graphs(self):
        ...
        logger.info(
            f"Capture draft{" extend" if self.draft_extend_attn_backend else ""} cuda graph begin. This can take up to several minutes. avail mem={before_mem:.2f} GB"
        )
        self.cuda_graph_runner = EAGLEDraftExtendCudaGraphRunner(self) if self.draft_extend_attn_backend else EAGLEDraftCudaGraphRunner(self)
        after_mem = get_available_gpu_memory(self.device, self.gpu_id)
        logger.info(
            f"Capture draft{" extend" if self.draft_extend_attn_backend else ""} cuda graph end. Time elapsed: {time.perf_counter() - tic:.2f} s. mem usage={(before_mem - after_mem):.2f} GB. avail mem={after_mem:.2f} GB."
        )
